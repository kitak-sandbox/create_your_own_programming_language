module Mio
    # Message is a tree of tokens produced when parsing.
    #   1 print.
    # is parsed to:
    #   Message.new("1",
    #               Message.new("print"))
    ...
    class Message < Object
        ...
        def call(receiver, context=receiver, *args)
            if @terminator
                # reset receiver to object at beginning of the chain.
                # eg.:
                #   hello there. yo
        ...
        # Parse a string into a tree of messages
        def self.parse(code)
        ...
        def self.parse_all(code, line)
            ...
            # Parsing code. Very similar to the Lexer we built for Awesome.
            while i < code.size
                case code[i..-1]
                when /\A("[^"]*")/, # string
                     /\A(\d+)/, # number
